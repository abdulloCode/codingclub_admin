// TeacherPanel.js boshida
const useRequireAuth = (requiredRole) => {
  const { isAuthenticated, user, isLoading } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    console.log('👨‍🏫 TeacherPanel useRequireAuth:', {
      isLoading,
      isAuthenticated,
      userRole: user?.role,
      requiredRole,
    ...
    // ✅ QAT'IY TEKSHIRUV: faqat teacher bo'lsa kirsin
    if (!isAuthenticated || user?.role !== requiredRole) {
      console.log(`❌ TeacherPanel: redirecting to teacher-login. User role: ${user?.role}, required: ${requiredRole}`);
      navigate('/teacher-login', { replace: true });
    }
  }, [isAuthenticated, user?.role, isLoading, navigate, requiredRole]);
...
export default function TeacherPanel() {
  ...
  const loadGroups = useCallback(async () => {
    try {
      ...
      // Filtr - faqat shu teacherning guruhlari
      const teacherId = user?.id || user?.userId;
      if (teacherId && grps.length > 0) {
  ...
  const loadSubmissions = useCallback(async (assignmentId) => {
    try {
      ...
      console.log('Submissions loaded for assignment:', assignmentId, data);
  ...
  useEffect(() => {
    ...
    loadAssignments();
    loadGroups();
  }, [authLoading, loadAssignments, loadGroups]);

  useEffect(() => {
    if (groups.length > 0) {
      loadTeacherAttendance();
    }
  }, [groups, loadTeacherAttendance]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('Xatolik yuz berdi: ' + (err.message || 'Server bilan bog\'lanib bo\'lmadi'));
    } finally {
      setModalLoading(false);
    }
  };

  const gradeSubmission = async (assignmentId, submissionId) => {
    const points = gradingValues[submissionId];
    ...
    try {
      await apiService.gradeHomework(assignmentId, { studentId: submissionId, points: parseInt(points) });
      alert('Muvaffaqiyatli baholandi!');
      loadSubmissions(assignmentId);
      setGradingValues(prev => ({ ...prev, [submissionId]: '' }));
    } catch (err) {
  ...
  if (authLoading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: D ? '#0a0a0f' : '#f8fafc' }}>
        <RotateCw size={40} color={BRAND} style={{ animation: 'spin 1s linear infinite' }} />
        <p style={{ color: D ? '#fff' : '#1e293b', marginLeft: 16 }}>Yuklanmoqda...</p>
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: bg }}>
        ...
        <div style={{
          ...
          {/* TOP NAVBAR - same as before, keeping existing code */}
          <div style={{
          ...
          <div style={{ padding: '24px' }}>
            {loading ? (
              ...
              <>
                {/* DASHBOARD SECTION - keep existing JSX */}
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                      <div style={{
                                        ...
                                        {(submission.studentName || submission.student?.name || "O'quvchi")[0].toUpperCase()}
                                      </div>
                                      <div>
                                        <p style={{ fontSize: '14px', fontWeight: 600, color: tx, margin: 0 }}>
                                          {submission.studentName || submission.student?.name || "O'quvchi"}
                                        </p>
                                        <p style={{ fontSize: '12px', color: mu, margin: 0 }}>
                                          {submission.submittedAt ? new Date(submission.submittedAt).toLocaleDateString('uz-UZ') : 'Noma\'lum'}
                                        </p>
                                      </div>
                                    </div>
                                    {submission.graded && submission.points !== undefined && (
                                      <div style={{
                                  ...
                                  {!submission.graded && (
                                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                      <input
                                        ...
                                        value={gradingValues[submission.id] || ''}
                                        onChange={(e) => setGradingValues(prev => ({ ...prev, [submission.id]: e.target.value }))}
                                      />
                                      <button
                                        onClick={() => gradeSubmission(selectedAssignment.id, submission.id)}
                                        className="btn-primary"
                ...
                {/* GROUPS SECTION - keep existing JSX */}
        ...
        {/* CREATE MODAL - keep existing code */}
        {showCreateModal && (
          <div style={{
