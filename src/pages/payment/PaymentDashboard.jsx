...
/* ─── CONFIRM MODAL ─────────────────────────────────────────────── */
// FIX: Replaced native confirm() and prompt() with React modals
      ...
      <div className="card" style={{ width: '100%', maxWidth: 400, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <div style={{
            ...
            background: `${DANGER}15`, display: 'flex',
            alignItems: 'center', justifyContent: 'center'
          }}>
            <AlertCircle size={20} color={DANGER} />
          </div>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: TEXT }}>Tasdiqlash</h3>
        </div>
        <p style={{ fontSize: 14, color: TEXT_MUTED, marginBottom: 20 }}>{message}</p>
  ...
  return (
    <div style={{
      ...
      <div className="card" style={{ width: '100%', maxWidth: 420, padding: 24 }}>
        <h3 style={{ fontSize: 16, fontWeight: 700, color: TEXT, marginBottom: 12 }}>
          Rad etish sababi
        </h3>
        <textarea
          ...
          <button
            ...
            style={{
              ...
              background: reason.trim()
                ? `linear-gradient(135deg, ${DANGER}, #b91c1c)`
                : `${DANGER}40`,
              color: '#fff', fontSize: 13, fontWeight: 600, border: 'none'
            }}>
            Rad etish
...
export default function AdminPayments() {
  ...
  // FIX: Separated stats from derived data to avoid stale closure & infinite loop
  ...
  // FIX: Replaced confirm()/prompt() with state-driven modals
  ...
  // FIX: fetchStats now receives paymentsList, studentsList, teachersList, groupsList
  // as arguments instead of reading from stale state — prevents stale closure & infinite loop
  const fetchStats = useCallback(async (paymentsList, studentsList, teachersList, groupsList) => {
    try {
      const today = new Date();
      const todayStr = today.toISOString().split('T')[0];
      const currentMonth = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}`;

      const [monthlyReport, dailyReport] = await Promise.allSettled([
        apiService.getPaymentReport(currentMonth),
        apiService.getDailyFinancialSummary(todayStr)
      ]);

      const monthly = monthlyReport.status === 'fulfilled' ? monthlyReport.value : null;
      const daily   = dailyReport.status   === 'fulfilled' ? dailyReport.value   : null;

      // FIX: Use passed arguments instead of stale state refs
      const totalStudentDebt = studentsList.reduce((sum, student) => {
        const paid = paymentsList
          .filter(p => p.toWhoId === student.id && p.status === 'approved')
          .reduce((ps, p) => ps + (p.amount || 0), 0);

        const group = groupsList.find(g => g.students?.some(s => s.id === student.id));
        const expected = group?.monthlyPrice || 0;
      ...
      const teacherCommissionTotal = teachersList.reduce((sum, teacher) => {
        const earnings = paymentsList
          .filter(p => p.fromWhoId === teacher.id && (p.dk === 'credit' || p.type === 'credit'))
          .reduce((te, p) => te + (p.amount || 0), 0);
        return sum + (earnings * (teacher.commissionPercentage || 0)) / 100;
      }, 0);
      ...
      setStats({
        ...
        approvedPayments:       paymentsList.filter(p => p.status === 'approved').length,
        rejectedPayments:       paymentsList.filter(p => p.status === 'rejected').length,
  ...
  }, []); // FIX: no dependencies — all data passed as args

  // FIX: Single coordinated fetch — fetches all data then computes stats once
  const fetchAll = useCallback(async () => {
    try {
      ...
      const [paymentsData, typesData, teachersData, studentsData, groupsData] =
        await Promise.all([
          apiService.getPayments(params),
          apiService.getPaymentTypes(true),
      ...
      const typesList     = Array.isArray(typesData)     ? typesData     : (typesData?.paymentTypes  || typesData?.data    || []);
      ...
      // FIX: Pass fresh data directly — no stale state risk
      await fetchStats(paymentsList, studentsList, teachersList, groupsList);
    } catch (error) {
  ...
  }, [selectedMonth, selectedGroup, selectedType, fetchStats]);
  ...
  // ─── FILTERING ────────────────────────────────────────────────
  // FIX: Consistent client-side filtering (server-side filters applied in fetchAll)
  useEffect(() => {
  ...
  const handleCreatePayment = async (e) => {
    ...
    try {
      // FIX: Convert amount from string to number before sending
      const payload = {
        ...paymentForm,
        amount: parseFloat(paymentForm.amount),
        // FIX: Include typeId in payload (was silently dropped before)
        typeId: paymentForm.typeId || undefined,
        groupId: paymentForm.groupId || undefined,
        toWho: paymentForm.toWho || undefined,
        lessonDate: paymentForm.lessonDate || paymentForm.date,
      };

      // FIX: Validate amount is a valid positive number before API call
  ...
  const handleApprovePayment = async (paymentId) => {
    try {
      ...
      showToast(error.message || "To'lovni tasdiqlashda xatolik yuz berdi", "error");
    }
  };

  // FIX: Replaced prompt() with RejectModal component
  ...
  const confirmReject = async (reason) => {
    ...
    try {
      ...
      showToast(error.message || "To'lovni rad etishda xatolik yuz berdi", "error");
    }
  };

  // FIX: Replaced confirm() with ConfirmModal component
  const handleDeletePayment = (paymentId) => {
    setConfirmModal({
      message: "Ushbu to'lovni o'chirishni tasdiqlaysizmi? Bu amalni ortga qaytarib bo'lmaydi.",
      onConfirm: async () => {
        ...
        try {
          ...
          showToast(error.message || "To'lovni o'chirishda xatolik yuz berdi", "error");
        }
      }
    });
  };

  const handleDeletePaymentType = (typeId) => {
    setConfirmModal({
      message: "Ushbu to'lov turini o'chirishni tasdiqlaysizmi?",
      onConfirm: async () => {
        setConfirmModal(null);
        try {
          ...
          showToast(error.message || "To'lov turini o'chirishda xatolik yuz berdi", "error");
        }
      }
    });
  };
  ...
  // FIX: Unified helper — checks both `type` and `dk` fields consistently
  const isCredit = (payment) =>
  ...
  const getTypeBadge = (payment) => {
    const credit = isCredit(payment);
    const config = credit
    ...
    <>
      ...
      <style>{`
        ...
        * { font-family: 'Inter', system-ui, -apple-system, sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .admin-payments { min-height: 100vh; background: ${BACKGROUND}; }
        ::-webkit-scrollbar { width: 6px; height: 6px; }
        ::-webkit-scrollbar-track { background: transparent; }
        ::-webkit-scrollbar-thumb { background: ${BORDER}; border-radius: 3px; }
        ::-webkit-scrollbar-thumb:hover { background: ${BRAND}40; }
      ...
      <div className="admin-payments">
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{
                ...
                background: CARD_BG2, borderRadius: 40,
                ...
                <input
                  ...
                  style={{
                    ...
                    outline: 'none', fontSize: 13, color: TEXT, width: 180
                  }}
                />
              </div>
              <button
        ...
        <main style={{ maxWidth: 1400, margin: '0 auto', padding: '24px' }}>
          {/* Tabs */}
          <div style={{
            display: 'flex', gap: 8, background: CARD_BG,
            borderRadius: 16, padding: '6px',
            border: `1px solid ${BORDER}`, marginBottom: 24, overflowX: 'auto'
          }}>
            {tabs.map(tab => {
              const Icon = tab.icon;
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 400 }}>
              <div style={{ textAlign: 'center' }}>
                <div className="animate-spin" style={{
                  ...
                  border: `3px solid ${BORDER}`, borderTopColor: BRAND, marginBottom: 16
                }} />
              ...
              {activeTab === 'overview' && (
                <div className="animate-fade-in">
                  <div style={{
                    ...
                    {[
                      { label: "Jami daromad",         value: fmtCurrency(stats.totalRevenue),      icon: TrendingUp,   color: SUCCESS,     trend: "+12%" },
                      { label: "Oylik daromad",         value: fmtCurrency(stats.monthlyRevenue),    icon: DollarSign,   color: INFO,        trend: "+8%"  },
                      { label: "Kunlik daromad",        value: fmtCurrency(stats.dailyRevenue),      icon: CreditCard,   color: WARNING,     trend: "+5%"  },
                      { label: "Jami to'lovlar",        value: stats.totalPayments,                  icon: FileText,     color: BRAND_LIGHT, trend: "+15%" },
                      { label: "O'quvchi qarzdorligi",  value: fmtCurrency(stats.totalStudentDebt),  icon: AlertCircle,  color: DANGER,      trend: "-3%"  },
                      { label: "To'langan summa",       value: fmtCurrency(stats.totalPaidAmount),   icon: CheckCircle,  color: BRAND,       trend: "+10%" }
                    ].map((stat, index) => (
                      <div key={index} className="card card-hover" style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                          ...
                          <span style={{
                            ...
                            background: `${SUCCESS}15`, color: SUCCESS, fontWeight: 600
                          }}>{stat.trend}</span>
                        </div>
                        <h3 style={{ fontSize: 28, fontWeight: 700, color: TEXT, marginBottom: 4 }}>
                          {stat.value}
                        </h3>
                        <p style={{ fontSize: 12, color: TEXT_MUTED, fontWeight: 500 }}>{stat.label}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="card" style={{ padding: '20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
                      {[
                        { icon: Plus,     label: "To'lov qo'shish",        color: SUCCESS,     onClick: () => setShowCreateModal(true) },
                        { icon: FileText, label: "To'lov turi yaratish",   color: INFO,        onClick: () => setShowTypeModal(true) },
                        { icon: Users,    label: "O'quvchilar",            color: BRAND_LIGHT, onClick: () => navigate('/students-panel') },
                        { icon: Download, label: "Hisobot yuklab olish",   color: WARNING,     onClick: () => setActiveTab('reports') }
                      ].map((action, index) => (
                        <button key={index} onClick={action.onClick} className="btn card-hover" style={{
              ...
              {activeTab === 'payments' && (
                <div className="animate-fade-in">
                  <div style={{
                    ...
                    <div>
                      <h2 style={{ fontSize: 20, fontWeight: 700, color: TEXT }}>Barcha to'lovlar</h2>
                      <p style={{ fontSize: 12, color: TEXT_MUTED }}>Jami {filteredPayments.length} ta yozuv</p>
                    </div>
                    <div style={{ display: 'flex', gap: 12 }}>
                      <button onClick={() => setShowCreateModal(true)} className="btn" style={{
                      ...
                      <button onClick={fetchAll} className="btn" style={{
                        width: 40, height: 40, borderRadius: 10,
                        background: 'transparent', border: `1px solid ${BORDER}`,
                  ...
                  <div className="card" style={{ padding: '16px', marginBottom: 20 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Oy</label>
                        <input type="month" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="input" />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Guruh</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>To'lov turi</label>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Holat</label>
                        ...
                        {paginatedPayments.map((payment, index) => (
                          ...
                          >
                            <div style={{
                              ...
                              background: isCredit(payment) ? `${SUCCESS}15` : `${DANGER}15`,
                              display: 'flex', alignItems: 'center',
                              justifyContent: 'center', flexShrink: 0
                            }}>
                              {isCredit(payment)
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                ...
                                {/* FIX: Pass full payment object to getTypeBadge */}
                            ...
                            <div style={{
                              ...
                              color: isCredit(payment) ? SUCCESS : DANGER,
                              flexShrink: 0
                            }}>
                              {isCredit(payment) ? '+' : '-'}{fmtCurrency(payment.amount)}
                            </div>
              ...
              {activeTab === 'types' && (
                <div className="animate-fade-in">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <div>
                      <h2 style={{ fontSize: 20, fontWeight: 700, color: TEXT }}>To'lov turlari</h2>
                      <p style={{ fontSize: 12, color: TEXT_MUTED }}>Jami {paymentTypes.length} ta to'lov turi</p>
                    </div>
                    <button onClick={() => setShowTypeModal(true)} className="btn" style={{
                      padding: '10px 20px', borderRadius: 10,
                      background: `linear-gradient(135deg, ${BRAND_DARK}, ${BRAND})`,
                      color: '#fff', fontSize: 13, fontWeight: 600,
                      display: 'flex', alignItems: 'center', gap: 8, border: 'none'
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
                    {paymentTypes.map((type, index) => (
                      <div key={type.id} className="card card-hover" style={{
                        padding: '20px',
                        animation: `fadeIn 0.3s ease ${index * 0.05}s both`
                      }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                          ...
                          <button onClick={() => handleDeletePaymentType(type.id)} className="btn" style={{
                        ...
                        <h3 style={{ fontSize: 16, fontWeight: 600, color: TEXT, marginBottom: 4 }}>{type.name}</h3>
                        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                          <span style={{
                            fontSize: 11, padding: '2px 8px', borderRadius: 4,
                            background: type.dk === 'credit' ? `${SUCCESS}15` : `${DANGER}15`,
                            color: type.dk === 'credit' ? SUCCESS : DANGER, fontWeight: 600
                          }}>
                            {type.dk === 'credit' ? 'Kirim' : 'Chiqim'}
                          </span>
                          <span style={{
                            fontSize: 11, padding: '2px 8px', borderRadius: 4,
                            background: `${INFO}15`, color: INFO, fontWeight: 600
                          }}>
                            {type.code}
                          </span>
                        </div>
                        {type.description && (
                          <p style={{ fontSize: 12, color: TEXT_MUTED }}>{type.description}</p>
                        )}
                      </div>
                    ))}
                    {paymentTypes.length === 0 && (
                      <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 60, color: TEXT_MUTED }}>
              ...
              {activeTab === 'reports' && (
                <div className="animate-fade-in">
                  <div style={{ marginBottom: 20 }}>
                    <h2 style={{ fontSize: 20, fontWeight: 700, color: TEXT }}>Moliyaviy hisobotlar</h2>
                    <p style={{ fontSize: 12, color: TEXT_MUTED }}>Tizim moliyaviy statistikasi va hisobotlari</p>
                  </div>
                    ...
                    {[
                      {
                        ...
                        icon: Calendar, color: INFO,
                        // FIX: Removed prompt() — use selectedMonth state instead
                      ...
                      },
                      {
                        ...
                        onClick: () => showToast(`Bugungi daromad: ${fmtCurrency(stats.dailyRevenue)}`)
                      },
                      {
                        title: "O'quvchi hisobotlari",
                        description: "Har o'quvchi to'lov tarixi",
                        icon: Users, color: BRAND_LIGHT,
                      ...
                      <div key={index} onClick={report.onClick} className="card card-hover" style={{
                        padding: '24px',
                        animation: `slideUp 0.4s ease ${index * 0.1}s both`
                      }}>
                        <div style={{
                        ...
                        <h3 style={{ fontSize: 16, fontWeight: 600, color: TEXT, marginBottom: 8 }}>{report.title}</h3>
                        <p style={{ fontSize: 12, color: TEXT_MUTED }}>{report.description}</p>
                      </div>
                    ))}
                  </div>

                  <div className="card" style={{ padding: '24px' }}>
                    <h3 style={{ fontSize: 16, fontWeight: 600, color: TEXT, marginBottom: 16 }}>Umumiy statistika</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                      {[
                        ...
                        <div key={index} style={{
                          ...
                          <p style={{ fontSize: 11, color: TEXT_MUTED, marginBottom: 8 }}>{stat.label}</p>
                          <p style={{ fontSize: 20, fontWeight: 700, color: TEXT }}>{stat.value}</p>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {/* FIX: React-based Confirm Modal (replaces confirm()) */}
      {confirmModal && (
        <ConfirmModal
      ...
      {/* FIX: React-based Reject Modal (replaces prompt()) */}
      {rejectModal && (
        <RejectModal
      ...
      {showCreateModal && (
        <div style={{
          ...
          <div className="card" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: TEXT }}>Yangi to'lov</h3>
                <p style={{ fontSize: 12, color: TEXT_MUTED, marginTop: 2 }}>To'lov ma'lumotlarini kiriting</p>
              </div>
              <button onClick={() => setShowCreateModal(false)} className="btn" style={{
                width: 32, height: 32, borderRadius: 8,
                background: 'transparent', border: `1px solid ${BORDER}`,
                color: TEXT_MUTED, display: 'flex',
                alignItems: 'center', justifyContent: 'center'
            ...
            <form onSubmit={handleCreatePayment} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                ...
                <div>
                  ...
                  {/* FIX: Added min="0.01" and step="any" for better number input UX */}
                  <input
                    required
                    type="number"
                    step="any"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Sana *</label>
                  <input
                    required type="date"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Dars sanasi</label>
                  <input
                    type="date"
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Kim uchun *</label>
                <select
                  required
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Guruh</label>
                <select
              ...
              {/* FIX: typeId is now included in form and submitted correctly */}
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>To'lov turi (kategoriya)</label>
                <select
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Tavsif</label>
                <textarea
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                <button type="button" onClick={() => setShowCreateModal(false)} className="btn" style={{
      ...
      {showTypeModal && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: 450, padding: '24px', animation: 'fadeIn 0.3s ease' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: TEXT }}>Yangi to'lov turi</h3>
                <p style={{ fontSize: 12, color: TEXT_MUTED, marginTop: 2 }}>To'lov turi ma'lumotlarini kiriting</p>
              </div>
              <button onClick={() => setShowTypeModal(false)} className="btn" style={{
                width: 32, height: 32, borderRadius: 8,
                background: 'transparent', border: `1px solid ${BORDER}`,
                color: TEXT_MUTED, display: 'flex',
                alignItems: 'center', justifyContent: 'center'
            ...
            <form onSubmit={handleCreatePaymentType} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Nomi *</label>
                <input
                  required type="text"
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Kod *</label>
                <input
                  required type="text"
              ...
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: TEXT_MUTED, marginBottom: 6 }}>Turi *</label>
                <select
                  required
                  value={typeForm.dk}
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                <button type="button" onClick={() => setShowTypeModal(false)} className="btn" style={{
                  flex: 1, padding: '12px', borderRadius: 10,
                  background: 'transparent', border: `1px solid ${BORDER}`,
                  color: TEXT_MUTED, fontSize: 13, fontWeight: 600
                }}>
      ...
      {showDetailsModal && selectedPayment && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: 500, padding: '24px', animation: 'fadeIn 0.3s ease' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: TEXT }}>To'lov tafsilotlari</h3>
                <p style={{ fontSize: 12, color: TEXT_MUTED, marginTop: 2 }}>#{selectedPayment.id?.slice(0, 8)}</p>
              </div>
              <button onClick={() => setShowDetailsModal(false)} className="btn" style={{
                width: 32, height: 32, borderRadius: 8,
                background: 'transparent', border: `1px solid ${BORDER}`,
                color: TEXT_MUTED, display: 'flex',
                alignItems: 'center', justifyContent: 'center'
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{
                padding: '16px', borderRadius: 12,
                background: isCredit(selectedPayment) ? `${SUCCESS}15` : `${DANGER}15`,
                textAlign: 'center'
              }}>
                <p style={{ fontSize: 11, color: TEXT_MUTED, marginBottom: 4 }}>
                  {isCredit(selectedPayment) ? 'Kirim' : 'Chiqim'}
                </p>
                <p style={{ fontSize: 28, fontWeight: 700, color: TEXT }}>
                  {isCredit(selectedPayment) ? '+' : '-'}{fmtCurrency(selectedPayment.amount)}
                </p>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  <div key={index}>
                    <p style={{ fontSize: 11, color: TEXT_MUTED, marginBottom: 4 }}>{field.label}</p>
                    <p style={{ fontSize: 14, fontWeight: 600, color: TEXT }}>{field.value}</p>
                  </div>
                ))}
              </div>
