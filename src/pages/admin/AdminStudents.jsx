...
/* ─── DETAIL PANEL ───────────────────────────────────────────── */
function DetailPanel({ s, groups, onEdit, onDelete, onClose, D }) {
  ...
  const rows  = [
    { icon: Mail,        label: "Email",      val: emailOf(s) },
    { icon: Phone,       label: "Telefon",    val: phoneOf(s) },
    { icon: BookOpen,    label: "Guruh",      val: groupName },
    { icon: Wallet,      label: "Balans",     val: s.balance ? `${s.balance.toLocaleString('uz-UZ')} UZS` : "0 UZS" },
  ].filter(r => r.val);
  ...
  return (
    <div style={{
      ...
      animation: "as-in 0.3s ease both",
    }}>
      <div style={{
        padding: 20,
        background: `linear-gradient(135deg,${BRAND_DIM},${BRAND_L})`,
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="as-btn" onClick={() => onEdit(s)} style={{
          ...
          <button className="as-btn" onClick={() => onDelete(s.id)} style={{
            flex: 1, padding: "9px", borderRadius: 11,
